<template>
  <div class="page-meetup">
    <h1 style="text-align: center">
      Meetup Page {{ meetupId }}
      <router-link :to="{ name: 'meetup', params: { meetupId: +meetupId + 1 } }"
        >Перейти к следующему митапу</router-link
      >
    </h1>
...
export default {
  ...
  beforeRouteEnter(to) {
    ...
    // 4. (vm) => {} - разрешает успешно переходить на маршрут И вызывает после перехода callback с экземпляром компонента
    // Во всех случаях можно возвращать значение асинхронно промисом
